...
{
  SDL_Surface *screen, *temp, *sprite, *grass, *fireball;
    int colorkey;
    ...
    fireball = SDL_DisplayFormat(temp);
    SDL_FreeSurface(temp);
 ...
 /* setup sprite colorkey and turn on RLE */
    colorkey = SDL_MapRGB(screen->format, 255, 0, 255);
    SDL_SetColorKey(fireball, SDL_SRCCOLORKEY | SDL_RLEACCEL, colorkey);
	...
	/* COLLISION BOULE DE FEU DANS L'HYPERESPACE */
	/* COPIE COLLER DE AU DESSUS *_* */

	if (fireballPosition.x < 0)
	  fireballPosition.x = SCREEN_WIDTH- SPRITE_SIZE;
	if (fireballPosition.x > SCREEN_WIDTH - SPRITE_SIZE) 
	  fireballPosition.x = SPRITE_SIZE ;
	if (fireballPosition.y < 0)
	  fireballPosition.y = SCREEN_HEIGHT-SPRITE_SIZE;
	if (fireballPosition.y > SCREEN_HEIGHT - SPRITE_SIZE) 
	  fireballPosition.y = SPRITE_SIZE;
	...
	{ 
	  ...
	  fireballImage.y = 0;
	  fireballImage.w = FIREBALL_SIZE;
	  fireballImage.h = FIREBALL_SIZE;
	  fireballImage.x = FIREBALL_SIZE;

	  SDL_BlitSurface(fireball, &fireballImage, screen, &fireballPosition);
	}
